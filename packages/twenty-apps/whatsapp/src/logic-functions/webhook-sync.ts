...
// create a new message and link it all together
  ...
  // are message participants reused?
  ...
  const senderInfo = contacts;
  const senderPhoneNumber = messages.from;
  let whatsAppPerson = await findPersonByFilter(coreClient, senderPhoneNumber, senderInfo.wa_id);
  if (!whatsAppPerson.people?.totalCount || whatsAppPerson.people?.totalCount === 0) {
    await createPerson(coreClient, senderPhoneNumber, senderInfo.profile.name);
    whatsAppPerson = await findPersonByFilter(coreClient, senderPhoneNumber, senderInfo.wa_id);
  }
  ...
  switch (messages.type) {
    ...
    case 'system': {
      await updatePerson(coreClient, whatsAppPerson.people?.edges[0].node.id, messages.system.body);
      break;
    }
    case 'text': {
