...
export const postInstallCore = async ({
  ...
  for (const seed of ENRICHMENT_WORKFLOW_SEEDS) {
    ...
    if (!isDefined(logicFunctionId)) {
      console.warn(
        `[crustdata] Skipping "${seed.workflowName}": logic function ${seed.logicFunctionUniversalIdentifier} is not installed.`,
      );
      continue;
    }
    ...
    try {
      ...
      console.warn(
        `[crustdata] Failed to seed "${seed.workflowName}": ${errorMessage}`,
        error,
      );
