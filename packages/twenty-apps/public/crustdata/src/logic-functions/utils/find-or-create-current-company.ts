...
import { CrustdataOperationError } from 'src/logic-functions/errors/crustdata-operation-error';
...
const findOrCreateUncachedCompany = async ({
  ...
  try {
    ...
    if (!isDefined(createdCompanyId)) {
      throw new CrustdataOperationError('Failed to create company: no id returned.');
    }
...
export const findOrCreateCurrentCompany = async ({
  ...
  const hasAnyCompanyMatchKey =
    isNonEmptyString(companyMatchKeys.crustdataId) ||
    isNonEmptyString(companyMatchKeys.website) ||
