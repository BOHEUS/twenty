...
import { ZOOMINFO_LOGIC_FUNCTION_CONSTANTS } from 'src/constants/universal-identifiers';
...
const handler = (input: EnrichInput) =>
  enrichCompaniesCore({ input: toBulkEnrichInput(input) });

export default defineLogicFunction({
  universalIdentifier:
    ZOOMINFO_LOGIC_FUNCTION_CONSTANTS.enrichCompanies.universalIdentifier,
  name: 'enrich-companies',
  description: 'Enrich one or more Company records with ZoomInfo firmographics',
  timeoutSeconds: 300,
  handler,
  httpRouteTriggerSettings: {
    path: ZOOMINFO_LOGIC_FUNCTION_CONSTANTS.enrichCompanies.path,
    httpMethod: 'POST',
    isAuthRequired: true,
  },
