...
describe('buildBulkEnrichmentSnackbar', () => {
  it('should report a run where every record was enriched', () => {
    expect(buildBulkEnrichmentSnackbar(bulkEnrichmentResult)).toEqual({
      message: 'Enriched 3 of 3 records.',
      variant: 'success',
  ...
  it('should detail the records that were not enriched', () => {
    expect(
      ...
      message: 'Enriched 1 of 3 records.',
      variant: 'success',
      detailedMessage:
        '1 not found in Apollo, 1 without a key Apollo can match on.',
    });
  });

  it('should turn the snackbar into an error when a record failed', () => {
    expect(
      buildBulkEnrichmentSnackbar({
        ...bulkEnrichmentResult,
        success: false,
      ...
      message: 'Enriched 0 of 1 record.',
      variant: 'error',
      detailedMessage: '1 failed.',
    });
  });
});
