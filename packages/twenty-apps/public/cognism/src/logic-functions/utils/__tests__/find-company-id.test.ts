...
describe('findCompanyId', () => {
  it('matches by cognismId first and does not query the lower-priority keys', async () => {
    const query = vi.fn((request: unknown) =>
      Promise.resolve(
        filterKeyOf(request) === 'cognismId'
          ? { companies: { edges: [{ node: { id: 'co-cognism' } }] } }
          : { companies: { edges: [] } },
      ),
    );
