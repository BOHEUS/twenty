...
const FAKE_COST_PER_REDEEM_DOLLARS = 0.1;
...
type RecordConfig = {
  ...
  exists?: boolean;
  hasIdentifier?: boolean;
  outcome?: CognismEnrichResult<FakeData>;
  updateFails?: boolean;
  buildFails?: boolean;
};
...
const buildHarness = (configs: RecordConfig[]) => {
  ...
  const enrichBatch = vi.fn(
    async (params: FakeParams[]): Promise<CognismEnrichResult<FakeData>[]> =>
      params.map(
        (param) =>
  ...
  const adapter: BatchEnrichmentAdapter<FakeNode, FakeData, FakeParams> = {
    ...
    costPerRedeemDollars: FAKE_COST_PER_REDEEM_DOLLARS,
    readRecords,
    extractParams: ({ node }) =>
...
describe('runBatchEnrichment', () => {
  ...
  it('writes NOT_FOUND and ERROR statuses with batched status updates', async () => {
    ...
    expect(harness.updateManyStatus).toHaveBeenCalledWith({
      ...
      recordIds: ['a', 'b'],
      data: {
        cognismEnrichmentStatus: 'NOT_FOUND',
        cognismLastEnrichedAt: expect.any(String),
      },
    });
  ...
  it('skips no-identifier records before the Cognism call', async () => {
    const harness = buildHarness([
  ...
  it('marks all attempted records as ERROR and writes the status when the batch Cognism call rejects', async () => {
    const harness = buildHarness([{ id: 'a' }, { id: 'b' }]);
    harness.enrichBatch.mockRejectedValueOnce(new Error('cognism down'));
    ...
    expect(result.results[0].error).toBe('cognism down');
    expect(harness.updateOne).not.toHaveBeenCalled();
    expect(harness.updateManyStatus).toHaveBeenCalledWith({
      client: CLIENT,
      recordIds: ['a', 'b'],
  ...
  it('chunks large id sets into separate read and Cognism calls', async () => {
  ...
  it('bills only matched outcomes after the Cognism call', async () => {
    const harness = buildHarness([
      { id: 'a' },
      { id: 'b' },
