...
describe('findOrCreateCurrentCompany', () => {
  it('returns undefined and issues no calls when Cognism gives no identifiers', async () => {
    const query = vi.fn();
    ...
    const result = await findOrCreateCurrentCompany({
      client,
      personData: {} as CognismPersonData,
  ...
  it('links an existing company matched by cognismId alone', async () => {
    const query = vi.fn(() =>
      Promise.resolve({
    ...
    const result = await findOrCreateCurrentCompany({
      client,
      personData: { account: { id: 'cognism-co-1' } as CognismPersonData },
      companyIdByMatchKeyCache: newCache(),
    });
  ...
  it('attempts a cognismId match but does not create without a name or website', async () => {
  ...
  it('creates a company from the Cognism data when none matches', async () => {
    const query = vi.fn(() => Promise.resolve({ companies: { edges: [] } }));
    ...
    const result = await findOrCreateCurrentCompany({
      client,
      personData: {
        account: { id: 'cognism-co-2', name: 'Acme', domain: 'acme.com' },
      } as CognismPersonData,
      companyIdByMatchKeyCache: newCache(),
    });
  ...
  it('re-finds the winner when a concurrent create hits a unique violation', async () => {
    ...
    const query = vi.fn((request: unknown) =>
      Promise.resolve(
        created && 'cognismId' in filterOf(request)
    ...
    const result = await findOrCreateCurrentCompany({
      client,
      personData: {
        account: { id: 'cognism-co-3', name: 'Acme' },
      } as CognismPersonData,
      companyIdByMatchKeyCache: newCache(),
    });
