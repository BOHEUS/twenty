...
const writeErrorStatus = async <TNode extends { id: string }, TData, TParams>({
  adapter,
  client,
  recordIds,
  enrichedAt,
...
const recordMatchedRecords = async <
...
const recordNotFoundRecords = async <
  TNode extends { id: string },
  TData,
  TParams,
>({
...
>({
  ...
  const nodeByRecordId = new Map(
    recordNodes.map((recordNode) => [recordNode.id, recordNode]),
  );
  ...
  let cognismEnrichmentOutcomes: CognismEnrichResult<TData>[];
  try {
    cognismEnrichmentOutcomes = await adapter.enrichBatch(
  ...
  await chargeRedeemedEnrichments({
    redeemedCount: cognismEnrichmentOutcomes.filter(
      (enrichmentOutcome) => enrichmentOutcome?.outcome === 'matched',
    ).length,
    costPerRedeemDollars: adapter.costPerRedeemDollars,
    resourceContext: `cognism/${adapter.objectNameSingular.toLowerCase()}`,
  });
  ...
  for (let index = 0; index < recordsToEnrich.length; index++) {
    ...
    const enrichmentOutcome = cognismEnrichmentOutcomes[index];
    ...
    if (
      ...
      const enrichmentErrorMessage = isDefined(enrichmentOutcome)
        ...
        : 'Cognism returned no response for this record.';
      resultById.set(
        recordId,
