...
describe('enrichCompaniesCore', () => {
  it('fills empty standard fields and writes cognism metadata via updateCompany on a match', async () => {
    ...
    expect(typeof captured.updateCompany?.cognismLastEnrichedAt).toBe('string');
    expect('domainName' in (captured.updateCompany ?? {})).toBe(false);
    expect('cognismMatchScore' in (captured.updateCompany ?? {})).toBe(false);
  });
  ...
  it('batches the Cognism request and the not-found status write across records', async () => {
    ...
    expect(enrichCompaniesMock).toHaveBeenCalledTimes(1);
    expect(captured.updateCompanies?.filter).toEqual({
      id: { in: ['c1', 'c2'] },
    });
