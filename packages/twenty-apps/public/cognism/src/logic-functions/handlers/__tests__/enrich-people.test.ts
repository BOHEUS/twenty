...
describe('enrichPeopleCore', () => {
  it('rejects an out-of-range match score before calling Cognism or writing records', async () => {
    const captured: Captured = {};
    const client = buildClient({ people: [PERSON_NODE_MOCK], captured });

    await expect(
      enrichPeopleCore({
        input: { records: [{ id: 'p1' }], minMatchScore: 101 },
        client,
      }),
    ).rejects.toThrow(
      'Minimum match score must be an integer between 0 and 100.',
    );
  ...
  it('fills empty standard fields and writes cognism metadata via updatePerson on a match', async () => {
    ...
    expect(typeof captured.updatePerson?.cognismLastEnrichedAt).toBe('string');
    expect(captured.updatePerson?.cognismRawPayload).toMatchObject({
      id: 'cognism1',
    });
  ...
  it('records NOT_FOUND and writes the status via a batched updatePeople', async () => {
    ...
    expect(Object.keys(captured.updatePeople?.data ?? {}).sort()).toEqual([
      'cognismEnrichmentStatus',
      'cognismLastEnrichedAt',
    ]);
  ...
  it('batches the Cognism request and the not-found status write across records', async () => {
    enrichPeopleMock.mockResolvedValue([
      { outcome: 'not_found', httpStatus: 404 },
      { outcome: 'not_found', httpStatus: 404 },
    ]);
